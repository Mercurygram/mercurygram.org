...
| Strip tracking parameters | Removes click-tracking parameters (`utm_*`, `fbclid`, `gclid`, `igsh` and about seventy more) from links you open, links you paste into the message field, links you copy out of the app, and links shared into Mercurygram from another app. Site-specific ones go too, where they are only tracking on their own site: `si` on YouTube and Spotify, `s`/`t` on X, the share ids on TikTok and Instagram's `stkn`, `ref`/`pd_rd_*` on Amazon, `ved` on Google Search. Parameters that carry real meaning are kept, such as Instagram's `img_index`. The rest of the address is untouched. | Off |
